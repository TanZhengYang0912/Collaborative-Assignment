...
export default function App() {
  ...
  // If GPS arrives before the map loads, store the position here and apply in onMapLoad
  const pendingPan = useRef(null);
  // Zoom + center saved just before fitBounds — restored when user clears
  ...
  // Sync night/day style to live map instance
  useEffect(() => {
    mapRef.current?.setOptions({ styles: isDark ? nightStyle : dayStyle });
  }, [isDark]);

  // GPS → pan map once → fetch 2 nearest restaurants from Supabase
  const locateAndLoad = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords: { latitude: lat, longitude: lng } }) => {
        ...
        if (mapRef.current) {
          // Map already loaded — pan immediately
          mapRef.current.panTo({ lat, lng });
          mapRef.current.setZoom(14);
        } else {
          // Map not loaded yet — store position, apply in onMapLoad
  ...
  // Step 1: tap marker → show InfoWindow only, no API call
  function handleSelect(restaurant) {
    setSelected(restaurant);
    setRoute(null);
  }

  // Step 2: "Start Navigation" → save current view, then fitBounds to full route
  async function handleNavigate() {
    if (!selected || !userPos) return;

    // Save zoom + center BEFORE fitBounds zooms out
    if (mapRef.current) {
      savedView.current = {
  ...
  // Clear selection + restore the zoom/center the user had before navigating
  ...
  const onMapLoad = useCallback((mapInstance) => {
    ...
    // If GPS already came back before the map finished loading
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", position: "relative" }}>
      <LoadScript googleMapsApiKey={import.meta.env.VITE_MAPS_BROWSER_KEY}>
        {/*
          defaultCenter / defaultZoom = uncontrolled props.
          They set the initial view only — re-renders never snap the map back.
          All subsequent panning/zooming is done imperatively via mapRef.
        */}
        <GoogleMap
