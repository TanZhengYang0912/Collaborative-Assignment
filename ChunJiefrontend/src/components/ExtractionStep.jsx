...
export default function ExtractionStep({ jobData, onBack, onReset }) {
  ...
  const buildCSV = () => {
    ...
    const dishes = Array.isArray(mergedRecord.signature_dishes)
      ? mergedRecord.signature_dishes.join(', ') : '';
    const cuisines = Array.isArray(mergedRecord.cuisine_types)
      ? mergedRecord.cuisine_types.join(', ') : '';
      
    const platform = (jobData.url || '').toLowerCase().includes('tiktok') ? 'TikTok' : 'YouTube';
    const lastUpdated = new Date().toISOString();

    const row = [
      mergedRecord.vendor_name        || '',
      mergedRecord.address            || '',
      mergedRecord.city               || '',
      mergedRecord.state              || '',
      mergedRecord.country            || 'Malaysia',
      '', // latitude (blank for later)
      '', // longitude (blank for later)
      cuisines,
      dishes,
      mergedRecord.price_range        || '',
      mergedRecord.sentiment_score    || '',
      '', // average_rating (blank)
      '', // review_count (blank)
      jobData.summary                 || '', // ai_review_summary
      mergedRecord.operating_hours_raw|| '',
      jobData.url                     || '', // source_video_url
      platform,
      lastUpdated
  ...
  // ── Download as CSV (Blob with Uint8Array BOM for Excel) ──
  const downloadCSV = () => {
    // 0xEF, 0xBB, 0xBF is the true UTF-8 BOM byte sequence
    const bom      = new Uint8Array([0xEF, 0xBB, 0xBF]);
    const content  = buildCSV();
    const filename = `${(mergedRecord.vendor_name || 'vendor').replace(/[^a-zA-Z0-9]/g, '_')}.csv`;
